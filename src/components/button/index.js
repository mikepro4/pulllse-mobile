...
const Button = ({
  ...
  const getButtonStyle = () => {
    return iconOnly ? styles.iconOnly : styles.buttonContainer;
  };
  ...
  const renderIcon = () => {
    ...
    if (label && icon) {
      return <View style={{ paddingRight: 5 }}>{iconRender}</View>;
    }
    if (icon) {
      return <Icon name={icon} style={{ fill: iconFill }} />;
    }
  };
  ...
  const renderStatusIcon = () => {
    return <Animated.View style={[styles.status, statusIconStyle]} />;
  };
  ...
  return (
    ...
    >
      <View style={styles.contentWrapper}>
        {renderIcon()}
        <CustomText style={{ fontSize: 14, color: textColor }}>
          {label}
        </CustomText>
        {renderIconRight()}
        {renderStatusIcon()}
