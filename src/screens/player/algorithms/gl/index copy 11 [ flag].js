...
function App() {
  ...
  const onContextCreate = (gl) => {
    ...
    gl.shaderSource(fragShader, `
      ...
      vec2 center = vec2(-0.5,1.5);
      vec4 bx = rectangle(uv, center, 0.,0., vec3(0));
      
        float fct=0.0,xx=0.2,yy=0.0; 
          
        for(int i=0;i<322;i++){
            xx +=0.04;
            fct= sin(2.0*gt+(uv.y-uv.x)*5.0)*0.015;;
          
            bx += rectangle(uv, center-vec2(-xx,yy), 0.02+fct, 0.04+fct,
                            rgb(uv.x-xx-1.,uv.y+xx-0.9,xx*0.8)*2.0);      
                
            if(xx> 1.1){xx=0.2; yy +=0.06;}
        }
            
      vec4 bg= vec4(rgb(0., 0.0, 0.0), 0.0);

      gl_FragColor = vec4(mix(bg, bx,bx.a));
