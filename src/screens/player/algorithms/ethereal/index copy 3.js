...
function App() {
  ...
  const onContextCreate = (gl) => {
    ...
    for (let i = 0; i < 1000; i++) {
      // Random positions
      verticesRef.current.push(Math.random(1) * 2 - 1, Math.random() * 2 - 1);
      // Random colors
  ...
  const frameTicker = useCallback(() => {
    ...
    const radius = 0.0001;
    const step = 0.0003 * 5.5 + 0.0001;
    const speed = 1.009;
    const friction = 0.01 * 0.8 + 0.1;
    ...
    if (gl && animationFrameId.current !== null) {
      ...
      for (let i = 0; i < verticesRef.current.length; i += 2) {
        const xIndex = i;
        const yIndex = i + 1;
        const centerX = 0; // The GLView coordinates center is (0,0)
        const centerY = 0;

        const angle = step * i + rotate.current;

        // const distance = Math.sqrt(
        //   Math.pow(verticesRef.current[xIndex] - centerX, 2) +
        //   Math.pow(verticesRef.current[yIndex] - centerY, 2)
        // );
  
  

        const t_radius = Math.cos(angle * 2.321 + freq * i) * radius * bold_rate + radius;
        // const tx = verticesRef.current[xIndex] + Math.cos(angle) * t_radius;
        // const ty = verticesRef.current[yIndex] + Math.sin(angle) * t_radius;
        ...
        // velocitiesRef.current[yIndex] *= friction;

        const tx = verticesRef.current[xIndex] + Math.cos(angle) * t_radius;
        const ty = verticesRef.current[yIndex] + Math.sin(angle) * t_radius;

        // Approach the target positions
        velocitiesRef.current[xIndex] += (tx - verticesRef.current[xIndex]) * speed;
        velocitiesRef.current[yIndex] += (ty - verticesRef.current[yIndex]) * speed;
