...
function App() {
  ...
  const onContextCreate = (gl) => {
    ...
    // Create a target
    const target = new Target(gl, widthRef.current / 8, widthRef.current, heightRef.current);
    targetsRef.current.push(target);

     // Generate points and assign a target
     for (let i = 0; i < 5000; i++) {
      pointsRef.current.push({
        ...
        vx: 0,
        vy: 0,
        target: targetsRef.current[i % targetsRef.current.length],
      });
    }
  ...
  const frameTicker = useCallback(() => {
    ...
    if (gl && animationFrameId.current !== null) {
      ...
      for (let i = 0; i < verticesRef.current.length; i += 2) {
        ...
        const forceY = 0.05 * Math.sin((yIndex + 1) * 0.1 * now);
