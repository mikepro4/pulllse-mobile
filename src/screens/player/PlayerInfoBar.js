...
import PulsePlayer from "../../components/pulse_player";
...
const PlayerInfoBar = () => {
  ...
  const getPulseTitle = () => {
    if (player.editedPulse && player.editedPulse.title) {
      if (!player.editedPulse.title || player.editedPulse.title.length == 0) {
        ...
        return (
          <CustomText
            style={{ fontSize: 18, fontFamily: "aeonik-", letterSpacing: 0.1 }}
          >
            {player.editedPulse.title}
          </CustomText>
        );
      }
    } else {
      if (player.originalPulse) {
        return (
          ...
          >
            {player.originalPulse.pulse.name}
          </CustomText>
        );
      } else {
        return (
  ...
  return (
    <Animated.View style={[styles.infoBarContainer, animatedStyles]}>
      {pulseRecording?.link ? (
        <PulsePlayer data={pulseRecording} />
      ) : (
